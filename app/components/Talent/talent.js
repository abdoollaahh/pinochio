...
const ItemTalentCard = ({ title, description, image, url }) => {
  ...
  return (
    <div className="relative col-span-2 m-4 overflow-hidden rounded-md transition hover:shadow-2xl hover:shadow-slate-600">
      <motion.div
        ...
        className="absolute left-0 top-0 flex  h-full w-full cursor-pointer flex-col  justify-end gap-4  p-8 font-semibold text-white transition-all duration-500 ease-in-out hover:bg-black/20 hover:backdrop-blur-[2px]  hover:backdrop-grayscale-[0.4]">
        <LayoutGroup>
          ...
          <AnimatePresence mode="wait">
            {hover && (
              <motion.div
                className="hidden text-3xl font-light shadow-2xl lg:block xl:text-3xl"
